...
The full reasoning is in `CLAUDE.md` and `KB/HANDOFF.md`. This is the operative set.
...
One logical change per commit. After every modification: `git add -A`, commit with what changed and
why, `git push origin main`. Never end a session with uncommitted work.
