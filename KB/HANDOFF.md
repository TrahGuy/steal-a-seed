...
The Mythic's flat saucer lotus is gone. It had 39 parts, a smiling face and a
Neon halo shell. In its place is option 2 of the owner's three concepts,
`art/creatures/gloomlotus/reference/gloomlotus-option2-walking-titan.jpg`;
option 1 and the lineup comparison are committed beside it.

  * **Where it lives:** a Tanglemire plant is a table of PartSpecs that
    `CreatureModel.replayPart` replays, so the model is
    `TanglemireForms.Plants.gloomlotus`.
      - The table is generated by the new `tools/art/gloomlotus_titan.luau`, a
        Studio-only tool that previews the specs and serializes them in the
        file's own format.
      - The file header and the table's comment say so, and say to regenerate
        rather than hand-edit.
  * **Supporting mass: four arched stilt roots.** Their bearings are 40, 132,
    222 and 316 degrees, so from the front the back pair peeks past the front
    pair.
      - Each root is one curve out of a `RootNexus` block, over a knee near 3.1
        studs and down to the ground. It is cut into `RootRise`, `RootArch`,
        `RootDescent` and `RootAnchor` beams that turn about 40 degrees at each
        joint.
      - The anchor lands on its inner corner; a flared `RootKnuckle` wedge takes
        the outer side.
      - A `RootMoss` cap on each arch, and a thin `PropRoot` inside it.
      - Studded swamp wood 68,52,40; moss 42,92,74.
      - Two `AerialRootlet` wedges hang under the nexus.
  * **Dominant mass:**
      - `TwistTrunk`, four slices (56,42,34). Each slice turns about 25 degrees
        on the one below and leans the other way, which makes the S.
      - Three Neon `ShelfFungus` discs (80,180,150), on three faces at three
        heights. In the third draft two of them sat level and read as eyes, and
        the brief wants no face.
      - Four weeping-moss wedges (52,108,80) hang from the outer petals'
        undersides. The two nearest dead right and dead left are named `Leaf`.
  * **Signature shape: the lotus and the wisp jewel.**
      - `LotusCalyx` in deep indigo-violet 50,38,70.
      - Six broad outer petals. Each is a flat seafoam blade (38,68,62) closed
        to a point by a violet diamond tip (`OuterPetal`, `OuterPetalTip`),
        tilted 18-24 degrees. They sit 30 degrees off the front, so the front
        camera looks between two of them at the jewel.
      - Six jagged, upturned inner petals (70,158,138): single wedges laid on
        their sides, alternating hands.
      - `WispJewel`, a Neon cube (80,245,215) balanced on one corner. It floats
        0.86 above the calyx, with its lowest point level with the inner tips.
  * **No face:** no Eye, Glint, Cheek or Smile parts, so `retrofitMireEye`
    finds nothing to add to.
  * **Effects:** `CreatureModel.retrofitGloomAura` is rewritten around the
    jewel.
      - `WispGlow`: a PointLight (80,245,215), Brightness 2.2, Range 14 at the
        drawn size, scaled with the plant.
      - `WispMotes`: one emitter, `sparkles_main.dds` at rate 16, rising, with
        LightEmission 1 and LightInfluence 0. Its attachment is turned back to
        world axes, because the gem stands on a corner.
      - The old haze and mote emitters and the `AuraRim` Highlight are gone,
        with the halo geometry they hung on.
  * **Budget:** 62 parts, 63 with Base, at the top of the 48-62 band.
      - roots 29 (the nexus plus seven parts per root), rootlets 2, trunk 4,
        fungi 3, calyx 1, outer petals 12, inner petals 6, moss 4, jewel 1;
      - 4 Neon parts, 1 light, 1 emitter, no Highlight.
  * **Height:** PreviewHeight goes from 6.81 to 7.33 (measured 7.326, at the
    jewel's top). SeedData's `Height` stays 6.82, so at every tier it ships at
    the height of the lotus it replaced.
  * **SeedData:** the gloomlotus comment is rewritten. Body, Crown and Accent
    are left as they were, but no part reads them any more; the portrait helper
    that would has no caller.
  * **Rig:** no part name carries a PlantSway leg word (Thigh, Knee, Foot,
    Stilt, Pillar ...). It moves like every other Tanglemire plant: PlantSway's
    whole-body bob and roll, plus the two `Leaf` moss strands swinging.

### Deviations and owner calls

  * **The roots do not step.** This walking titan slides its feet under a
    bobbing body. PlantSway's leg rig swings a limb rigidly about a hip above
    it; an arch whose foot lands 3 studs out from its crown is a different
    shape, and it was not tried. The brief asked only that the Leaf rig work.
  * **No pulse.** The brief describes the jewel as pulsing. Its light and motes
    are steady; a pulse would need a new per-frame animation channel.
  * **The jewel is a cube on its corner, not a long diamond.** A long
    bipyramid needs corner wedges, and the replay has no CornerWedge shape. The
    cube reads as a faceted gem from every side, for one part.
  * **The outer petals are blades with diamond tips.** The brief described
    cupped petals. A V-folded triangle version was built first and read as a
    star or thistle from above.
  * **The feet dip into the bed.** Measured on the garden plant:
      - while walking, the lowest root point ran from -0.258 to +0.364;
      - while standing, the idle lean held it between -0.200 and -0.068.
    Every Tanglemire plant rides the same body motion.
  * **Colossal still overruns the plot.** It is 54.68 wide x 58.82 deep;
    Plot_01's `Ground` is 48.0 wide. The lotus it replaced was 66.48 x 61.11.
  * **The placement disc does not match it.** PlantPlace's one formula for
    every species, `FrameHeight x 0.46 x Girth x 1.5`, gives 3.86 at Tiny under
    a 6.53-wide body and 51.86 at Colossal under 54.68. The old lotus was 8.31
    wide at Tiny. Unchanged.
  * **Not committed:** `.agents/skills/primitive-organic-sculpting/` (named in
    this brief) and `colossal-titan-sculpting/` are still untracked. They are
    not this task's files.

### Verified

  * `rojo build` clean. All 18 specs pass:
      - LeaderstatsSpec 28, SoundLevelSpec 28, CarryHandsSpec 16,
        HeldRestoreSpec 31, DebugTeleportSpec 8;
      - TutorialPodSpec 263, GuardianConfiscateSpec 88, PlantInfoSpec 45 (350
        grown combinations), WeaponSpec 107;
      - TutorialSpec 93, PlotSpec 65, SpeedSpec 332, CycleSpec 31, MillSignSpec
        (no failures);
      - BatSwingSpec 93/93, BatClearanceSpec 939/939, AlmanacClaimSpec 75,
        StarbloomLimbSpec 71.
  * **The data:**
      - `TanglemireForms`, reloaded from source, matches the generator: 62
        entries, worst difference 4.8e-7 in position, 5e-7 in rotation and
        4.8e-7 in size, and no name, shape, colour or material mismatch.
      - Bogbonnet, Crookreed, Snapmoss, Lanterncap, the pod and Miremaw are
        identical to the module before the edit.
      - `CreatureModel` compiles, with the new retrofit and the old call gone.
  * **Design passes in Edit**, four drafts in a preview folder that is now
    deleted:
      - first: angular knee roots, plank petals, a jewel hidden by the inner
        petals;
      - second: curved roots, and folded triangle petals that read as a star;
      - third: blade petals with violet tips, but a front petal masked the
        jewel and two fungi read as eyes;
      - fourth: petals 30 degrees off the front, the jewel raised, the fungi
        spread and the violet deepened. Checked from the front, a raised
        three-quarter, the side and a bloom close-up, plus an all-black
        silhouette.
  * **The old lotus, measured first** (in Edit): 44 parts at Tiny (39, plus
    Base and the eye retrofit), 8.31 x 6.82 x 7.60, 2 emitters, 1 Highlight, no
    lights. At Colossal, 66.48 x 51.87 x 61.11.
  * **Play, every tier** through CreatureModel with fresh modules:

    | Tier | Frame | Box w x h x d | Light Range | Placement disc |
    |---|---|---|---|---|
    | Tiny | 6.82 | 6.53 x 6.82 x 7.02 | 13.0 | 3.86 |
    | Big | 9.41 | 9.13 x 9.41 x 9.81 | 18.0 | 5.84 |
    | Huge | 12.96 | 12.73 x 12.95 x 13.68 | 24.7 | 8.76 |
    | Mega | 18.07 | 18.00 x 18.06 x 19.35 | 34.5 | 13.34 |
    | Giant | 25.23 | 25.53 x 25.22 x 27.45 | 48.2 | 20.37 |
    | Titan | 34.78 | 35.89 x 34.76 x 38.59 | 66.4 | 31.20 |
    | Colossal | 51.83 | 54.68 x 51.81 x 58.82 | 99.0 | 51.86 |

    At every tier: 63 parts, 1 light, 1 emitter at rate 16, no Highlight and 2
    `Leaf`. All four anchors and knuckles, and all four prop roots, sit at
    0.0000; top over frame is 0.999. Ranges above 60 are the property as set;
    the engine draws no further than 60.
  * **Play, the Almanac:** Tanglemire shelf, card 5 is Gloomlotus. The
    showcase reads Gloomlotus / Mythic / +$24/s, and its stage model is 66
    parts: the 63 plus the three-disc pedestal, with the jewel, 1 light and 1
    emitter. Screenshot: the arches, bloom and jewel read at card size.
  * **Play, the owner's garden:** PlantService restored the owner's tier-1
    Gloomlotus into Plot_01 from the new data (63 parts, 1 light, 1 emitter,
    wandering).
      - Screenshots from 14 studs and from 49 studs across the plot: next to a
        tier-6 Supernovus, the arches, bloom and jewel glow still pick it out.
      - A walk leg set on the client only, leaving the save untouched: it walked
        with both `Leaf` strands swinging (up to 32.5 degrees against the body),
        and the jewel's light pooled turquoise on the bed. A server leg arrived
        mid-sample, so it covered 10.6 studs rather than 5.
  * **Console:** clean through the Play session.

### Not verified

  * The Tanglemire biome's own lighting: every look in Play was on the plot, in
    daylight.
  * Night, a phone, or more than one client.
